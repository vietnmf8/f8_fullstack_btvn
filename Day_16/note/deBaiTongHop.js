...
customers.forEach(customer => {     //customer -> {id: 1, name: "John Doe"}
    // Filter order -> mảng mới customerOrders
    const customerOrders = orders.filter((object) => object.customerId === customer.id)
    let customerOrderItems = [];
    // Group Items of Order
    customerOrders.forEach(customerOrder => {   //{ id: 1001, customerId: 1, items: [ [Object], [Object] ] }
        // customerOrder.items -> [ { productId: 101, quantity: 2 }, { productId: 102, quantity: 1 } ]
